...
const ARRAY_FIELDS = ['key_products', 'competitors']
...
function sanitizeData(data: any) {
  ...
  Object.keys(sanitized).forEach(key => {
    if (ARRAY_FIELDS.includes(key)) {
      // Ensure array fields are always arrays or null
      if (!sanitized[key]) {
        sanitized[key] = null
      } else if (!Array.isArray(sanitized[key])) {
        try {
          // If it's a string, try to parse it as an array
          sanitized[key] = sanitized[key].split(',').map((item: string) => item.trim())
        } catch (e) {
      ...
      // For all other string fields, just trim
